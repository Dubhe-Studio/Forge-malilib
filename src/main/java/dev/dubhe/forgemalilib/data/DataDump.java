package dev.dubhe.forgemalilib.data;
...
public class DataDump
{
    ...
    private boolean checkData(String... data)
    {
        ...
        for (int i = 0; i < data.length; i++)
        {
            if (data[i] == null)
            {
                ForgeMaliLib.logger.warn("null value at column index {} on row '{}'", i, this.rowDataToString(data));
                valid = false;
            }
    ...
    public static File dumpDataToFile(File dir, String fileNameBase, String fileNameExtension, List<String> lines)
    {
        ...
        if (dir.exists() == false)
        {
            ...
            {
                ForgeMaliLib.logger.error("dumpDataToFile(): Failed to create the configuration directory", e);
                return null;
            }
        }
        ...
        if (outFile.exists())
        {
            ForgeMaliLib.logger.error("dumpDataToFile(): Failed to create data dump file '{}', one already exists", fileName);
            return null;
        }
        ...
        {
            ForgeMaliLib.logger.error("dumpDataToFile(): Failed to create data dump file '{}'", fileName, e);
            return null;
        }
        ...
        {
            ForgeMaliLib.logger.error("dumpDataToFile(): Exception while writing data dump to file '{}'", fileName, e);
        }
    ...
    public static void printDataToLogger(List<String> lines)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ForgeMaliLib.logger.info(lines.get(i));
        }
    }
